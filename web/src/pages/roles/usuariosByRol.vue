<template>
  <div class="q-pa-md">
    ...
    <q-table title="Usuarios por rol" :loading="loading" :rows="usuarios" :columns="columns" row-key="name"
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      loading.value = true;
      const idrol = '' + route.query.idrol;
      const res = await usuarioService.usuariosByRol(idrol).then((e) => e).catch((e) => e);
      usuarios.value = res.usuariosByRol;
      // console.log(res);
      loading.value = false
    });
    return {
