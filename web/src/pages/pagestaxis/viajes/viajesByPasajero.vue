<template>
  <div class="q-pa-md">
    ...
    <q-table title="Viajes por pasajero" :loading="loading" :rows="viajes" :columns="columns"
      row-key="name" hide-pagination :rows-per-page-options="[0]" :filter="filter">

      <template v-slot:top-right>
        <q-select v-model="conductor" :options="usuarios" option-label="nombres" option-value="id" outlined label="Seleccione pasajero:" lazy-rules class="full-width" dense :rules="[
...
export default {
  ...
  setup() {
    ...
    async function listar(){
      ...
      loading.value = true;
      const res = await viajesServices.viajesByPasajero(conductor.value.id).then((e) => e).catch((e) => e);
      viajes.value = res.viajesByPasajero;
      loading.value = false;
    }
