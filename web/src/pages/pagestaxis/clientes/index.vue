<template>
  <div class="q-pa-md">
    <q-table title="Clientes / Pasajeros" :loading="loading" :rows="conductores" :columns="columns"
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      loading.value = true;
      const res = await conductoresServices.clientes().then((e) => e).catch((e) => e);
      conductores.value = res.clientes;
      loading.value = false
    });
    return {
      store,
